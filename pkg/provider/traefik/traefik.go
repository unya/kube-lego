...
var _ kubelego.IngressProvider = &Traefik{}
...
func New(kl kubelego.KubeLego) *Traefik {
	return &Traefik{
		kubelego:        kl,
		usedByNamespace: map[string]bool{},
	}
...
func (p *Traefik) Log() (log *logrus.Entry) {
	return p.kubelego.Log().WithField("context", "provider").WithField("provider", "traefik")
}

func (p *Traefik) Reset() (err error) {
	p.Log().Debug("reset")
...
func (p *Traefik) Finalize() (err error) {
...
func (p *Traefik) removeServices() (err error) {
	// TODO implement me
	return nil
}

func (p *Traefik) updateServices() (err error) {
...
func (p *Traefik) updateService(namespace string) (err error) {
	var svc kubelego.Service = service.New(p.kubelego, namespace, p.kubelego.LegoServiceNameTraefik())
...
func (p *Traefik) Process(ingObj kubelego.Ingress) (err error) {
...
func (p *Traefik) getHTTPIngressPath() k8sExtensions.HTTPIngressPath {
	return k8sExtensions.HTTPIngressPath{
		Path: challengePath,
		Backend: k8sExtensions.IngressBackend{
			ServiceName: p.kubelego.LegoServiceNameTraefik(),
